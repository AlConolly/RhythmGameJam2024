...
namespace RhythmEngine.Examples
{
    ...
    public class SongEditor : MonoBehaviour
    {
        ...
        [Space]
        [SerializeField] private float BeatsKeyX = -10f; // The mininimum height of the beats parent
        ...
        private void CreateBeatVisuals(int beatCount)
        {
            for (int i = 0; i < beatCount; i++)
            {
                ...
                beat.localEulerAngles= new Vector3(0, 0, 90);
                beat.name = $"Beat {i}, Bar{Math.Floor(i / 16d)}";
                beat.localPosition = new Vector3(i * BeatSpacing, 0f, 0f);
            }
        }
        ...
        {
            var y = LanePositions[gridPosition.y];
            var x = gridPosition.x * BeatSpacing + NoteOnBeatHeight;
        ...
        private Vector2Int TimeToGridPosition(double time, int lane)
        {
            ...
            return new Vector2Int((int)beat,lane);
        }
        ...
        {
            // This is the max x position that the beats parent can have.
            var maxPosition = BeatsKeyX - BeatSpacing * _rawBeatCount;
            ...
            var beatsPosition = Mathf.Lerp(BeatsKeyX, maxPosition, songFraction);
            BeatsParent.localPosition = new Vector3(beatsPosition, 0f, 0f);
        }
